<template>
  <q-page class="detail-page">
          ...
          <q-tab-panels v-model="mainTab" animated class="tab-panels">
            ...
            <q-tab-panel name="services" class="tab-panel-padded">
              ...
              <div v-else class="services-grid">
                <div v-for="svc in queueServices" :key="svc.queue_service_id" class="service-card soft-card">
                  ...
                  <q-btn v-if="canManage" flat round dense icon="close" size="sm" class="service-card__remove" @click.stop="removeQueueService(svc)">
            ...
            <q-tab-panel name="tokens" class="tab-panel-padded">
              ...
              <div v-else class="list-items">
                <div v-for="code in accessCodes" :key="code.id" class="list-item">
                  <div class="list-item-info">
                  ...
                  <div class="list-item-side">
                    <q-btn flat round dense icon="content_copy" size="sm" @click="copyToClipboard(code.code)"><q-tooltip>Copiar código</q-tooltip></q-btn>
    ...
    <ContextMenu v-model="codeMenuOpen" :items="codeMenuItems" :position="codeMenuPos" :title="codeMenuTarget?.code" @select="onCodeMenuSelect" />
    ...
    <q-dialog v-model="showAddProfDialog" persistent>
      <q-card class="dialog-card">
        ...
        <q-card-section>
          <p class="text-muted q-mb-md">Selecione um profissional do estabelecimento:</p>
          <div v-if="estProfessionals.length === 0" class="empty-state-sm"><p>Nenhum profissional disponível</p></div>
          <div v-else class="list-items">
            <div
            ...
            >
              <div class="list-item-info">
                ...
                <div class="list-item-details">
                  ...
                  <span class="list-item-meta">{{ ep.user_email }}</span>
                </div>
              </div>
      ...
      <q-card class="dialog-card" style="min-width: 460px;">
        ...
        <q-card-section>
          ...
          <template v-else>
            ...
            <q-input v-model="svcForm.icon" label="Ícone (Material Icon)" outlined dense class="q-mb-md" hint="Ex: medical_services, cut, spa">
...
export default defineComponent({
  ...
  setup() {
    ...
    const openAddPersonDialog = () => {
      ...
      addToStatus.value = flowTab.value === 'completed' ? 'done' : (flowTab.value || 'waiting')
    ...
    const addPersonFromDialog = async () => {
      ...
      try {
        ...
        $q.notify({ type: 'positive', message: `${foundUser.value.name} adicionado(a) à fila` })
    ...
    const openAddProfDialog = () => {
      selectedEstProf.value = null
      fetchEstProfessionals()
    ...
    const removeProfessional = (prof) => {
      $q.dialog({
        ...
        message: `Remover ${prof.user_name} desta fila?`,
        cancel: { flat: true, label: 'Cancelar', noCaps: true },
        ok: { color: 'negative', label: 'Remover', noCaps: true },
      }).onOk(async () => {
        try {
          await api.delete(`/queues/${queueId.value}/queue-professionals/${prof.id}`)
          $q.notify({ type: 'positive', message: 'Profissional removido' })
          fetchQueueProfessionals()
        } catch { $q.notify({ type: 'negative', message: 'Erro ao remover' }) }
      })
    }
    ...
    const saveServiceAction = async () => {
      ...
      try {
        if (svcDialogMode.value === 'select') {
          // Batch link existing
          await api.post(`/queues/${queueId.value}/services`, { service_ids: selectedServiceIds.value })
          $q.notify({ type: 'positive', message: `${selectedServiceIds.value.length} serviço(s) adicionado(s) à fila` })
        } else {
          // Create new + link
    ...
    const removeQueueService = (svc) => {
      $q.dialog({
        title: 'Remover Serviço',
        message: `Remover "${svc.name}" desta fila?`,
        cancel: { flat: true, label: 'Cancelar', noCaps: true },
        ok: { color: 'negative', label: 'Remover', noCaps: true },
      }).onOk(async () => {
        try {
          await api.delete(`/queues/${queueId.value}/services/${svc.service_id}`)
          $q.notify({ type: 'positive', message: 'Serviço removido da fila' })
          fetchQueueServices()
        } catch { $q.notify({ type: 'negative', message: 'Erro ao remover serviço' }) }
      })
    }
    ...
    const entryMenuItems = computed(() => {
      ...
      if (t === 'waiting') {
        return [
          ...
          { separator: true },
          { key: 'remove', icon: 'person_remove', label: 'Remover da fila', danger: true, action: () => confirmRemoveEntry(id, entry.user_name) },
        ]
      }
      ...
      if (t === 'serving') {
        return [
          ...
          { separator: true },
          { key: 'remove', icon: 'cancel', label: 'Cancelar atendimento', danger: true, action: () => confirmRemoveEntry(id, entry.user_name) },
        ]
      }
    ...
    const codeMenuItems = computed(() => {
      ...
      return [
        ...
        { key: 'delete', icon: 'delete', label: 'Excluir', danger: true, action: () => deleteCode(c.id) },
      ]
    })
    ...
    const confirmRemoveEntry = (entryId, name) => {
      $q.dialog({
        ...
        message: `Tem certeza que deseja remover ${name || 'esta pessoa'} da fila?`,
        cancel: { flat: true, label: 'Cancelar', noCaps: true },
        ok: { color: 'negative', label: 'Remover', noCaps: true },
      }).onOk(() => removeEntry(entryId))
    }

    const removeEntry = async (entryId) => {
      try {
        await api.delete(`/queues/entries/${entryId}`)
        $q.notify({ type: 'positive', message: 'Pessoa removida da fila' })
        fetchData()
      } catch (err) { $q.notify({ type: 'negative', message: err.response?.data?.error?.message || 'Erro ao remover' }) }
    }

    const batchRemoveSelected = async (tab) => {
      const ids = tab === 'waiting' ? [...selectedWaiting.value] : [...selectedServing.value]
      if (!ids.length) return
      $q.dialog({
        title: 'Remover Selecionados',
        message: `Remover ${ids.length} pessoa(s) da fila?`,
        cancel: { flat: true, label: 'Cancelar', noCaps: true },
        ok: { color: 'negative', label: 'Remover', noCaps: true },
      }).onOk(async () => {
        ...
        try {
          ...
          if (tab === 'waiting') selectedWaiting.value = []
          else selectedServing.value = []
          fetchData()
        } catch (err) { $q.notify({ type: 'negative', message: err.response?.data?.error?.message || 'Erro ao remover' }) }
    ...
    const deleteCode = (codeId) => {
      $q.dialog({
        ...
        message: 'Tem certeza que deseja excluir este código de acesso?',
        cancel: { flat: true, label: 'Cancelar', noCaps: true },
        ok: { color: 'negative', label: 'Excluir', noCaps: true },
      }).onOk(async () => {
        try {
          await api.delete(`/queues/${queueId.value}/access-codes/${codeId}`)
          $q.notify({ type: 'positive', message: 'Código excluído' })
          fetchAccessCodes()
        } catch { $q.notify({ type: 'negative', message: 'Erro ao excluir código' }) }
      })
    }
    ...
    // ── Lazy tab loading ────────────────────────────────
    watch(mainTab, (tab) => {
      if (tab === 'professionals' && queueProfessionals.value.length === 0) fetchQueueProfessionals()
      if (tab === 'services' && queueServices.value.length === 0) fetchQueueServices()
      if (tab === 'tokens' && accessCodes.value.length === 0) fetchAccessCodes()
    ...
    return {
      ...
      // Services
      queueServices, showAddServiceDialog, svcDialogMode, selectedServiceIds, svcForm,
      availableEstServices, openAddServiceDialog, saveServiceAction, removeQueueService, toggleServiceSelection,
      // Dialogs
      ...
      // Code context menu
      codeMenuOpen, codeMenuPos, codeMenuTarget, codeMenuItems, openCodeMenu, onCodeMenuSelect,
      // Profile preview
      ...
      openCreateCode, openEditCode, saveCode, copyToClipboard,
      // Reports
      reportPeriod, reportData, periodOptions, formattedHourlyData, fetchReports,
      ...
      confirmRemoveEntry, removeEntry, batchRemoveSelected,
...
.service-card {
  ...
  &:hover .service-card__remove { opacity: 1; }
  ...
  &__remove { opacity: 0; transition: opacity 0.15s; position: absolute; top: 8px; right: 8px; color: var(--qm-text-muted); }
}
